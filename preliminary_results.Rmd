...
1. Explore the effect of the size of the gene pool (i.e., genetic diversity) on response of the population to an IRS intervention.
2. Seasonality is incorporated into the dynamics.
3. IRS is applied yearly from the end of the dry sesaon to the end of the wet season (days 120-300) for 2, 5 or 10 years. IRS coverage is 90%. The intervention increases the mortality of adult hosts and is implemented with a deterministic model.

# Example of a full time series
Immune selection at high genetic diversity, and high transmission. Blue line is a 5-year IRS.

```{r plot complete time series, echo=FALSE, fig.width=16,fig.height=9,dpi=300,out.width="1920px",out.height="1080px"}
library(tidyverse)
library(magrittr)
PS39_S_001 <- get_data(parameter_space = '39', scenario = 'S', experiment = '001', run = 1, host_age_structure = F)[[1]]
PS39_S_003 <- get_data(parameter_space = '39', scenario = 'S', experiment = '003', run = 1, host_age_structure = F)[[1]]
d <- rbind(PS39_S_001,PS39_S_003)
d %>% 
  select(-year, -month, -n_infected) %>% 
...
```{r parameter space, cache=TRUE, include=FALSE}
scenario <- 'S'
PS <- '36'
...
```{r plot comparison experiments, echo=FALSE, fig.width=16,fig.height=9,dpi=300,out.width="1920px",out.height="1080px"}
exp_comparison %>%
  select(-year, -month, -n_infected) %>% 
  filter(time>time_range[1]&time<time_range[2]) %>%
  gather(variable, value, -time, -exp, -PS, -scenario, -run, -pop_id) %>% 
...
This is an example with a 5-year IRS.

```{r Load_S_E002, cache=TRUE, include=FALSE}
scenario <- 'S'
...
```{r plot comparison selection, echo=FALSE, fig.width=16,fig.height=9,dpi=300,out.width="1920px",out.height="1080px"}
ps_comparison %>%
...
```{r load_summary_stats_S, cache=TRUE, include=FALSE}
setwd('~/Documents/malaria_interventions_data/')
scenario <- 'S'
...
```{r plot time to extinction, echo=FALSE, fig.width=16,fig.height=9,dpi=300,out.width="1920px",out.height="1080px"}
intervention_stats %>% 
  left_join(subset(design, select=c(PS,BITING_RATE_MEAN,N_GENES_INITIAL)), by='PS') %>% 
...
# Pobability of extinction as a proportion of runs which went extinct
```{r prob. of extinction, echo=FALSE, fig.width=16,fig.height=9,dpi=300,out.width="1920px",out.height="1080px"}
intervention_stats %>% 
  left_join(subset(design, select=c(PS,BITING_RATE_MEAN,N_GENES_INITIAL)), by='PS') %>% 
...
# Ratio of pre- vs post-IRS, compared to control
Calculate the ratio of the value of variables POST-intervention compared to control. For example, we can say that intervention has reduced the prevalence by 50% compared to control in a given parameter space and experiment. The post-intervention period starts 1 year after lifting the IRS, to avoid the transience.

```{r pre vs post-IRS, echo=FALSE, fig.width=16,fig.height=9,dpi=300,out.width="1920px",out.height="1080px"}
...
Compare the response of each of these scenarios to increasing IRS levels.

```{r scenario comparison data set, cache=TRUE, include=FALSE}
PS <- '36'
...
```{plot scenarios and experiments, echo=FALSE, fig.width=16,fig.height=9,dpi=300,out.width="1920px",out.height="1080px"}
scenario_comparison %>%
