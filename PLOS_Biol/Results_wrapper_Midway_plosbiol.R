...
for (i in 1:nrow(experiments)){
  ...
  x <- get_modularity_results(PS,scenario,run,cutoff_prob,folder = folder)
...
for (i in 1:nrow(experiments)){
  ...
  cutoff_prob <- experiments$cutoff_prob[i]
  folder <- experiments$folder[i]
  x <- get_temporal_diversity(PS,scenario,run,cutoff_prob,folder = folder)
...
for (i in 1:nrow(experiments)){
  ...
  cutoff_prob <- experiments$cutoff_prob[i]
  folder <- experiments$folder[i]
  x <- get_mFst(PS,scenario,run,cutoff_prob,folder = folder)
  mFst <- rbind(mFst,x)
}
mFst <- as.tibble(mFst) 
